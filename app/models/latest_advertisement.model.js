...
Advertisement.create = (newAdv, result) => {
  sql.query("INSERT INTO latest_advertisement SET ?", newAdv, (err, res) => {
    if (err) {
    ...
    console.log("Created latest_Advertisement: ", { item_code: res.insertAdv, ...newAdv });
   result(null, { item_code: res.insertAdv, ...newAdv });
  });
};

Advertisement.findById = (id, result) => {
  sql.query(`SELECT * FROM latest_Advertisement WHERE id = ${id}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Advertisement.getAll = (title_code, result) => {
  let query = "SELECT * FROM latest_Advertisement";
...
Advertisement.getAllPublished = result => {
  sql.query("SELECT * FROM latest_Advertisement where published=1 order by advt_no desc, item_code desc limit 4", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Advertisement.updateById = (id, advertisement, result) => {
  sql.query(
    "UPDATE latest_Advertisement SET advt_no = ?, link = ?, details = ?, published = ? WHERE item_code = ?",
...
Advertisement.remove = (id, advertisement) => {
  sql.query("DELETE FROM latest_Advertisement WHERE id = ?", id, (err, res) => {
    if (err) {
      console.log("error: ", err);
...
Advertisement.removeAll = advertisement => {
  sql.query("DELETE FROM latest_Advertisement", (err, res) => {
    if (err) {
      console.log("error: ", err);
      advertisement(null, err);
      return;
