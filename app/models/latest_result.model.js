...
Result.create = (newResult, result) => {
  sql.query("INSERT INTO latest_result SET ?", newResult, (err, res) => {
    if (err) {
...
Result.findById = (advt_no, result) => {
  sql.query(`SELECT * FROM latest_result WHERE published=1 and advt_no = ${advt_no}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Result.getAll = (title_code, result) => {
  let query = "SELECT * FROM  latest_result  where  published=1";
  console.log("500");
  if (title_code) {
...
Result.getAllPublished = result => {
  sql.query("SELECT * FROM latest_result where published=1 order by item_code desc limit 4", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Result.updateById = (id, Result, result) => {
  sql.query(
    "UPDATE latest_result SET advt_no = ?, link = ?, details = ?, published = ? WHERE item_code = ?",
...
Result.remove = (id, result) => {
  sql.query("DELETE FROM latest_result WHERE id = ?", id, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Result.removeAll = result => {
  sql.query("DELETE FROM latest_result", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
